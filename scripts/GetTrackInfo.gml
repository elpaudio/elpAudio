...
tag=FMODGetTagName()
//show_message(tag)
if  tag == "TITLE" /*or tag='TRK' or tag='TT2' or tag='TIT2' */global.songtitle = string_replace_all(string(FMODGetTagData()),'#','//');
else if tag == "ARTIST"/* or tag='TP1' or tag='TPE1'*/ global.songartist = string_replace_all(string(FMODGetTagData()),'#','//');
...
if tag=="GENRE" global.songgenre=GetSongGenre(string(FMODGetTagData()));
else if tag=="TCON" global.songgenre=GetSongGenre(string(FMODGetTagData()));}

else if tag=="APIC" global.songimg=string(FMODGetTagData());

//show_message(string(FMODGetTagData()));
//f=buffer_create() buffer_write_string(f,FMODGetTagData()) buffer_save(f,'test.txt') buffer_destroy(f)
}
