...
AddFileAction(2010,"ds_list_delete(global.list,global.current) global.current=global.current mod ds_list_size(global.list)-1 if variable_global_exists('musicsound') {FMODInstanceStop(global.playing) FMODSoundFree(global.musicsound) global.playing=-1 global.musicsound=-1} MetadataClear() if global.curpreloaded!=-1 global.curpreloaded=-1 if global.preloaded {FMODSoundFree(global.preloaded) global.preloaded=-1} eaPlay(global.current);")
AddFileAction(2011,"MusicImmediatelyStop() i=0 bb=ds_list_find_value(global.list,global.current) ll=ds_list_create() repeat(ds_list_size(global.list)) {if string_lower(filename_path(ds_list_find_value(global.list,i)))==string_lower(filename_path(bb)) ds_list_add(ll,ds_list_find_value(global.list,i)) i+=1} i=0 repeat(ds_list_size(ll)) {ds_list_delete(global.list,ds_list_find_index(global.list,ds_list_find_value(ll,i)))i+=1} ds_list_clear(ll) ds_list_destroy(ll) global.current=0 if ds_list_empty(global.list) HandlePlaylistLoad()")
AddFileAction(2012,"MusicImmediatelyStop() global.current=0 ds_list_clear(global.list) HandlePlaylistLoad()")
#define Alarm_2
/*"/*'/**//* YYD ACTION
lib_id=1
action_id=603
